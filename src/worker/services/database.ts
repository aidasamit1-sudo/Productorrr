export class DatabaseService {
  ...
  async createUser(userData: {
    ...
    const { success } = await this.db
      ...
      .bind(userData.id, userData.email, userData.name || null, userData.picture || null)
      .run();
  ...
  async logActivity(
    ...
    return await this.db
      ...
      .bind(userId, activityType, JSON.stringify(data || {}), ipAddress || null)
      .run();
  }
  ...
  async getImages(userId: string, page = 1, limit = 20) {
    ...
    const [images, countResult] = await Promise.all([
      ...
      this.db
        .prepare(`SELECT COUNT(*) as total FROM generated_images WHERE user_id = ?`)
        .bind(userId)
        .first(),
    ]);
